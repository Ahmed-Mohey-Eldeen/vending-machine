 ...
 u8 DIO_ReadPinVal(Pin_Type Pin_ID /*,u8 * ReadBufferPTR*/ )
{
   ...
   //*ReadBufferPTR = GET_BIT(*DIO_RegPIN[Port_Index],Pin_Index);
	return GET_BIT(*DIO_RegPIN[Port_Index],Pin_Index);
...
void DIO_Toggle_Pin(DIO_PORTS Port , u8 Pin)
 {
	 Pin_Type Pin_Index  = Pin % 8;
 	switch (Port)
 	{
 		case 0 : TOGGLE_BIT( PORTA , Pin_Index ) ; break;
 		case 1 : TOGGLE_BIT( PORTB , Pin_Index ) ; break;
 		case 2 : TOGGLE_BIT( PORTC , Pin_Index ) ; break;
 		case 3 : TOGGLE_BIT( PORTD , Pin_Index ) ; break;
 		default		: 								  break;
 	}
